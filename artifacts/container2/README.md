# Container 2 — Analysis & Risk Routing Subprocess

This container models and orchestrates the automated classification and risk analysis of
...
1. **Classification Agent**: Categorizes the type of anomaly.
2. **Anomaly Risk Agent**: Computes the numerical risk score.

A downstream **Fast-Track Gateway** evaluates these values and decides whether to route the anomaly for fully autonomous resolution or flags it for human oversight.
...
- [agent_risk.yaml](./agent_risk.yaml): Configuration for the Anomaly Risk Agent (Agent Builder).
...
graph TD
    ...
    Classify -->|Category + Confidence| Risk[Anomaly Risk Agent]
...
- **Outputs**:
  - `incidentId` (string)
  - `category`: Map to exactly one of `GRADIENT_COLLAPSE`, `LOSS_DIVERGENCE`, `HARDWARE_FAULT`, `RESOURCE_RISK`.
  - `confidence` (number 0.0 to 1.0): Classification confidence.
  - `reasoning` (string): Cognitive step-by-step logic.

### 2. Anomaly Risk Agent (`agent_risk.yaml`)
- **Inputs**: Canonical `IncidentReport` JSON.
- **Rules (1:1 Modeling)**:
  - Baseline risk = `0.05`.
  ...
  - Criticality zone factor: Multiply by `1.5` if `safetyZone` is `high-cost-gpu`, `production-bound`, `public-benchmark`, or `long-running`.
...
The Exclusive Gateway uses the outputs of the Classification Agent (`confidence`) and Anomaly Risk Agent (`riskScore`) to route:
